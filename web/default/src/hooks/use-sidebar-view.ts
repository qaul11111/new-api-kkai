 ...
 *   visibility (admin-only group and per-item `requiredRole`).
 *
 * `useSidebarData` already applies the `useSidebarConfig` admin × user
 * `sidebar_modules` overlay, so no config filtering is repeated here.
